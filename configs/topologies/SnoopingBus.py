...
class SnoopingBus(SimpleTopology):
    ...
    def makeTopology(self, options, network, IntLink, ExtLink, Router):
        ...
        xbar = Router(
            router_id=0,
            num_processor=options.num_cpus,
            tdm_slot_width=options.tdm_slot_width,
            resp_bus_slot_width=options.resp_bus_slot_width,
        )

        network.routers = xbar

        ext_links = [
            ExtLink(link_id=i, ext_node=n, int_node=xbar, latency=link_latency)
